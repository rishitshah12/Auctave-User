                    ...
                    {displayCategories.map(cat => {
                        ...
                        return (
                            <button key={cat.name} onClick={() => setSelectedGarmentCategory(cat.name)} className="flex-shrink-0 flex flex-col items-center justify-start space-y-2 p-1 transition-transform hover:scale-105 group w-24 text-center">
                                ...
                                <span className={`font-semibold text-xs transition-colors ${isSelected ? 'text-[#c20c0b]' : 'text-gray-600 dark:text-gray-400'}`}>{cat.name}</span>
                            </button>
                        );
                    })}
                </div>
            ...
            <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl shadow-2xl z-50 transform transition-transform duration-500 ease-in-out ${showFilterPanel ? 'translate-x-0' : 'translate-x-[120%]'}`}>
                <div className="flex flex-col h-full text-gray-800 dark:text-gray-200">
                    ...
                    <div className="flex-grow p-6 space-y-6 overflow-y-auto">
                        ...
                        <div>
                            <label htmlFor="moq" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Max. MOQ: {filters.maxMoq.toLocaleString()} units</label>
                        ...
                        <div>
                         ...
                         <div className="pt-6 border-t dark:border-gray-700 grid grid-cols-2 gap-4">
                            <button onClick={clearFilters} className="w-full flex items-center justify-center gap-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 py-3 rounded-lg font-semibold hover:bg-gray-300 dark:hover:bg-gray-600 transition-all"><Trash2 size={16} /> Clear All</button>
    ...
    const NotificationBell: FC = () => {
        ...
        return (
            <div className="relative mr-1 md:mr-2" ref={dropdownRef}>
                <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-full bg-white dark:bg-gray-800 shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors relative text-gray-600 dark:text-gray-300">
                    <Bell size={20} />
                    {notificationCount > 0 && (
                                ...
                                notifications.map(quote => (
                                    ...
                                    >
                                        <div className="flex items-center gap-2 overflow-hidden w-full">
                                            ...
                                            <p className="text-sm text-gray-700 dark:text-gray-300 truncate w-full">
        ...
        <div ref={profileDropdownRef} className="relative">
            ...
            {isProfileDropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-900/95 dark:backdrop-blur-xl rounded-md shadow-lg py-1 z-20 animate-fade-in border border-gray-100 dark:border-white/10">
                    <button onClick={() => { handleSetCurrentPage('profile'); setIsProfileDropdownOpen(false); }} className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center"><UserIcon size={16} className="mr-2" /> My Profile</button>
        ...
        <MainLayout {...props}>
            ...
            <section className="mb-6 sticky top-0 bg-white/80 dark:bg-gray-900/60 backdrop-blur-md py-3 z-30 -mx-4 px-4 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8 transition-colors">
                <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide max-w-7xl mx-auto">
                    <button onClick={() => setShowFilterPanel(true)} className="flex-shrink-0 px-4 py-2 border rounded-lg text-sm font-semibold transition-colors bg-white dark:bg-gray-900/50 border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-200 flex items-center gap-2"><SlidersHorizontal size={16} />Filters</button>
                    {quickFilters.map(filter => {
                        const isActive = isQuickFilterActive(filter.type, filter.value);
                        return (<button key={filter.name} onClick={() => handleQuickFilter(filter.type, filter.value)} className={`flex-shrink-0 px-4 py-2 border rounded-lg text-sm font-semibold transition-colors ${isActive ? 'bg-[#c20c0b] text-white border-[#c20c0b]' : 'bg-white dark:bg-gray-900/50 border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-200'}`}>{filter.name}</button>)
                    })}
                </div>
            </section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {isFiltering ? (
                        ...
                        <div className="col-span-full text-center py-12 bg-white dark:bg-gray-900/40 dark:backdrop-blur-md rounded-2xl shadow-sm border border-gray-200 dark:border-white/10">
                            ...
                            <p className="text-gray-600 dark:text-gray-300 font-semibold mt-4">No Factories Found</p>
