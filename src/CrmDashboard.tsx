...
export default function CrmDashboard({ callGeminiAPI, handleSetCurrentPage }: CrmDashboardProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (user) {
                // Fetch Orders
                const { data: orders, error: orderError } = await crmService.getOrdersByClient(user.id);
                if (orderError) {
                    ...
                    if (orders.length > 0) setActiveOrderKey(orders[0].id);
                }
